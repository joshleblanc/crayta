	...
	self:Schedule(
		function()
			...
			while true do
				...
				if (self.properties.roundIndex == 0 or #self:GetUsers() < self.properties.minPlayersToPlay or self.properties.alwaysDoLobby) and self.properties.maxLobbyTime > 0 then
...
function GameScript:DoLobbySchedule()
	...
	while not enoughPlayers do
		...
		GetWorld():ForEachUser(self.newUserCallback)
...
function GameScript:DoPlaySchedule()
	...
	GetWorld():ForEachUser(self.newUserCallback, true)
	...
	while GetWorld():GetServerTime() < self.endTime do
		Wait(1.0)
	end
...
function GameScript:DoResultsSchedule()
	...
	while GetWorld():GetServerTime() < stateStartTime + self.properties.maxResultsTime do
		Wait(1.0)
	end
